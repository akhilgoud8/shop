...
// ══════════════════ MAIN HOMEPAGE ══════════════════
const BEST_SELLERS = [{"emoji": "📱", "name": "Samsung Galaxy S24 Ultra 5G 256GB Phantom Black", "price": 89999, "original": 109999, "rating": 4.5, "reviews": 12430, "badge": "deal", "off": 18}, {"emoji": "🎧", "name": "Sony WH-1000XM5 Wireless Noise Cancelling Headphones", "price": 24990, "original": 34990, "rating": 4.7, "reviews": 8210, "badge": "prime", "off": 29}, {"emoji": "💻", "name": "Apple MacBook Air M3 13-inch 8GB RAM 256GB SSD", "price": 114900, "original": 129900, "rating": 4.8, "reviews": 5670, "badge": "new", "off": 12}, {"emoji": "⌚", "name": "Apple Watch Series 10 GPS 42mm Starlight Aluminium", "price": 39900, "original": 45900, "rating": 4.6, "reviews": 3890, "badge": "deal", "off": 13}, {"emoji": "📷", "name": "Canon EOS R50 Mirrorless Camera with 18-45mm Lens Kit", "price": 67990, "original": 79995, "rating": 4.4, "reviews": 2140, "badge": "", "off": 15}, {"emoji": "🎮", "name": "Sony PlayStation 5 Slim Digital Edition Console", "price": 44990, "original": 54990, "rating": 4.8, "reviews": 21000, "badge": "deal", "off": 18}, {"emoji": "🖥️", "name": "LG 27GN800-B 27\" UltraGear QHD 144Hz Gaming Monitor", "price": 22990, "original": 31990, "rating": 4.5, "reviews": 6780, "badge": "prime", "off": 28}, {"emoji": "🔊", "name": "JBL Flip 6 Portable Wireless Bluetooth Speaker IPX7", "price": 7499, "original": 11999, "rating": 4.6, "reviews": 34500, "badge": "deal", "off": 37}];
const FLASH_DEALS = [{"emoji": "👟", "name": "Nike Air Max 270 Running Shoes", "price": 5999, "off": "60% off", "progress": 72}, {"emoji": "📺", "name": "Samsung 55\" 4K QLED Smart TV", "price": 49990, "off": "45% off", "progress": 41}, {"emoji": "🧴", "name": "Minimalist 10% Niacinamide Serum Pack of 3", "price": 599, "off": "55% off", "progress": 88}, {"emoji": "☕", "name": "Nespresso Vertuo Next Coffee Machine", "price": 11990, "off": "35% off", "progress": 27}, {"emoji": "🎒", "name": "Wildcraft 55L Hiking Backpack Waterproof", "price": 1999, "off": "50% off", "progress": 63}, {"emoji": "🌡️", "name": "Omron HEM-7120 Automatic Blood Pressure Monitor", "price": 1299, "off": "42% off", "progress": 55}];
const MINI_ITEMS = [{"emoji": "📖", "name": "Atomic Habits by James Clear", "price": 399}, {"emoji": "🍳", "name": "Non-Stick Kadhai Set 3-Piece", "price": 1299}, {"emoji": "👕", "name": "Men's Cotton T-Shirt Pack of 5", "price": 799}, {"emoji": "🧘", "name": "Yoga Mat Anti-Slip 6mm Thick", "price": 699}, {"emoji": "🔦", "name": "LED Torch Rechargeable 3000mAh", "price": 349}, {"emoji": "🌱", "name": "Indoor Plant Combo Set of 4", "price": 599}, {"emoji": "🧩", "name": "Rubik's Speed Cube 3x3 Professional", "price": 449}, {"emoji": "🎸", "name": "Acoustic Guitar Beginner Pack", "price": 3499}];

async function renderProducts() {
  const grid = document.getElementById('productsGrid');
  const products = await fetchProducts('main', BEST_SELLERS);
  grid.innerHTML = products.map(productCard).join('');
}

function renderDeals() {
  const grid = document.getElementById('dealsGrid');
  grid.innerHTML = FLASH_DEALS.map(d => `
    <div class="deal-card" onclick="addToCart({emoji:'${d.emoji}', name:${JSON.stringify('')} , price:${d.price}})">
      <div class="deal-img">${d.emoji}<span class="badge badge-deal" style="top:8px;left:8px">${d.off}</span></div>
      <div class="deal-body">
        <div class="deal-name">${d.name}</div>
        <div class="deal-price">${fmtINR(d.price)}</div>
        <div class="deal-progress">
          <div class="progress-bar"><div class="progress-fill" style="width:${d.progress}%"></div></div>
          <div class="progress-label">${d.progress}% claimed</div>
        </div>
      </div>
    </div>
  `).join('');
...
function renderMini() {
  ...
  grid.innerHTML = MINI_ITEMS.map(m => `
    <div class="mini-card" onclick="addToCart({emoji:'${m.emoji}', name:${JSON.stringify(m.name)}, price:${m.price}})">
      <div class="mini-img">${m.emoji}</div>
      <div class="mini-name">${m.name}</div>
      <div class="mini-price">${fmtINR(m.price)}</div>
    </div>
  `).join('');
}

// ── COUNTDOWN ──
let totalSecs = 2 * 3600 + 47 * 60 + 33;
function tick() {
  totalSecs--;
  if (totalSecs < 0) totalSecs = 6 * 3600;
